...
plot_cdf = function(dist, data, x, ...){
  plot(ecdf(data), main = "CDF")
  curve(get_F(dist= dist, x, ...), add = TRUE, lwd = 3, col = 'orange')
  abline(v = get_q(dist, x = .5, ...), col = 'red', lwd = 3)
  legend("bottomright",
     c("eCDF", "estimated CDF", "median"),
...
plot_cdf(dist = "norm", data = d1$gh, x = x, lh, ch)
...
plot_cdf(dist = "norm", data = d1$gh, x = x, mean_hat, sd_hat)
...
plot_cdf(dist = "gamma", data = d1$gh, x = x,  ch, lh)
plot_qq(dist = "gamma", probs = ((1:200)/200), data = d1$gh,ch, lh)
mtext(sprintf("Gamma with estimated parameters shape = %s, rate = %s by mm", ch %>% round(., 3), lh %>% round(., 3)), side = 3, line = 0, outer = T)
```
<br>
...
plot_cdf(dist = "gamma", data = d1$gh, x = x, shape_hat, rate_hat)
...
find_shape = function(data, a, b){
  dif = c()
for (i in seq(a, b, by = 0.001)){
  ...
  out = data.frame(shape_candidate = seq(a, b, by = 0.001), dif)
  f = approxfun(out$shape_candidate, out$dif)
  f(seq(a, b, by = 0.001))
}

result = find_shape(d1$gh %>% as.numeric, a = 0.1, b = 35)
shape_hat = seq(0.1, 35, by = 0.001)[which.min(result)]
...
plot_cdf(dist = "weibull", data = d1$gh, x = x, shape_hat, scale_hat)
...
plot_cdf(dist = "norm", data = d1$ht, x = x, lh, ch)
...
plot_cdf(dist = "norm", data = d1$ht, x = x, mean_hat, sd_hat)
...
plot_cdf(dist = "gamma", data = d1$ht, x = x, ch, lh)
...
plot_cdf(dist = "gamma", data = d1$ht, x = x, shape_hat, rate_hat)
...
# weibull
# mm
result_ht = find_shape(d1$ht %>% as.numeric, a = 0.1, b = 50)
shape_hat = seq(0.1, 50, by = 0.001)[which.min(result_ht)]
x_bar = mean(d1$ht)
scale_hat = x_bar / gamma(1 + 1 / shape_hat)
```
...
plot_cdf(dist = "weibull", data = d1$ht, x = x,  shape_hat, scale_hat)
