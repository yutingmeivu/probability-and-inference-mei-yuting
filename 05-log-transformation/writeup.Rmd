...
sum$log_pdf = dgamma(x_, shape = 3, scale = 1) %>% log()
sum$log_cdf = pgamma(x_, shape = 3, scale = 1) %>% log()
sum = data.frame(sum)

# plot pdf, median, mean of gamma distribution and gamma distribution of x after log trnasformation
plot_ = function(df){
  ...
  length()){
    ...
    geom_line() +
      geom_hline(aes(yintercept = mean(df[, i], na.rm = T)),
          colour = "gray", linetype ="longdash", size = .8) +
      geom_hline(aes(yintercept = median(df[, i], na.rm = T)),
          colour = "#B4DAD8", linetype ="longdash", size = .8) +
      labs(x = "log transformation of x") +
      geom_text(x = max(x_)- 5, y = max(df[, i]), label = sprintf("mean = %s, median = %s", mean(df[, i], na.rm = T) %>% round(., 3), median(df[, i], na.rm = T) %>% round(., 3))))
  }
  else{
    print(df %>%
    ggplot(aes(x = x_, y = df[, i])) +
...
# for(i in 1:1000){
#   x <- rgamma(n, shape = 3, scale = 1)
# exp_of_logx[i] = x %>% log() %>% mean()
# log_of_expx[i] = x %>% mean() %>% log()
# }
# 
# data.frame(exp_of_logx, log_of_expx) %>%
#   pivot_longer(c("exp_of_logx", "log_of_expx"), names_to = "type", values_to = "expectation") %>%
#   ggplot(aes(x = c(seq(1, 1000), seq(1, 1000)), y = expectation, color = type)) +
#   geom_point()
# 
# data.frame(exp_of_logx, log_of_expx) %>%
#   summarise(difference = exp_of_logx - log_of_expx) %>%
...
sum_ln$log_pdf = dlnorm(x_, meanlog = -1, sdlog = 1, log =T)
sum_ln$log_cdf = plnorm(x_, meanlog = -1, sdlog = 1, log =T)
...
sum_uni$log_pdf = dunif(x_, min = 0, max = 12, log = T)
sum_uni$log_cdf = punif(x_, min = 0, max = 12, log = T)
...
according the Taylor Series, we have $e^x \geq 1+x$.
<br>
